...
import { ModelPicker } from "@infinite/components/model-picker";
import { ChannelEditorDrawer } from "@infinite/components/layout/channel-editor-drawer";
import { ConfigPromptSources } from "@infinite/components/layout/config-prompt-sources";
import { ConfigLocalStorage } from "@infinite/components/layout/config-local-storage";
import type { AppLocale } from "@infinite/i18n";
import { exportAppConfig, importAppConfig } from "@infinite/services/config-file";
import { syncAppDataToWebdav, type AppSyncDomainKey, type AppSyncProgressEvent } from "@infinite/services/app-sync";
import { isCanvasMindRuntimeChannel } from "@infinite/services/canvasmind-model-runtime";
import { testWebdavConnection, WEBDAV_MANIFEST_FILE_NAME } from "@infinite/services/webdav-sync";
import { audioFormatOptions, audioVoiceOptions, normalizeAudioSpeedValue } from "@infinite/lib/audio-generation";
import { createModelChannel, modelOptionsFromChannels, normalizeModelOptionValue, selectableModelsByCapability, useConfigStore, useEffectiveConfig, type AiConfig, type ApiCallFormat, type ConfigTabKey, type ModelCapability, type ModelChannel } from "@infinite/stores/use-config-store";
...
export function AppConfigPanel({
    ...
    const finishConfig = () => {
        const ready = effectiveConfig.channels.some((channel) => channel.baseUrl.trim() && channel.apiKey.trim() && channel.models.length);
                                        ...
                                        <div key={channel.id} className="flex items-center justify-between gap-3 rounded-lg border border-stone-200 px-4 py-3 dark:border-stone-800">
                                            <div className="min-w-0">
                                                <div className="flex min-w-0 items-center gap-2">
                                                    ...
                                                    {isCanvasMindRuntimeChannel(channel) ? <span className="shrink-0 rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-medium text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300">{t("config.channels.synced")}</span> : null}
                                                </div>
                                                <div className="mt-1 truncate text-xs text-stone-500">
                                                    {apiFormatLabel(channel.apiFormat, t)} · {t("config.channels.modelCount", { count: channel.models.length })} · {isCanvasMindRuntimeChannel(channel) ? t("config.channels.canvasMindManaged") : channel.baseUrl || t("config.channels.missingUrl")}
                                                </div>
                                            </div>
                                            {!isCanvasMindRuntimeChannel(channel) ? (
