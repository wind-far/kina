...
export const useWorkflowOrchestrator = () => {
  ...
  const clearWatchers = () => {
    activeWatchers.forEach((stop: any) => stop())
  ...
  const waitForConfigComplete = (configNodeId: string) => {
    return new Promise((resolve, reject) => {
      ...
      const node = nodes.value.find((n: any) => n.id === configNodeId)
      if (checkNode(node)) return

      stopWatcher = watch(
        () => nodes.value.find((n: any) => n.id === configNodeId),
        (node: any) => checkNode(node),
        { deep: true }
      )
  ...
  const waitForOutputReady = (outputNodeId: string) => {
    return new Promise((resolve, reject) => {
      ...
      const node = nodes.value.find((n: any) => n.id === outputNodeId)
      if (checkNode(node)) return

      stopWatcher = watch(
        () => nodes.value.find((n: any) => n.id === outputNodeId),
        (node: any) => checkNode(node),
        { deep: true }
      )
      activeWatchers.push(stopWatcher)
  ...
  // 文生图工作流
  const executeTextToImage = async (imagePrompt: string, position: any) => {
    const sp = 400
  ...
  // 文生图生视频工作流
  const executeTextToImageToVideo = async (imagePrompt: string, videoPrompt: string, position: any) => {
    const sp = 400
    ...
    try {
      ...
      const imageNodeId = await waitForConfigComplete(imgConfigId) as string
      await waitForOutputReady(imageNodeId)

      const imageNode = nodes.value.find((n: any) => n.id === imageNodeId)
  ...
  // 分镜工作流
  const executeStoryboard = async (character: any, shots: any[], position: any) => {
    ...
    try {
      ...
      const charImageId = await waitForConfigComplete(charConfigId) as string
      ...
      for (let i = 0; i < shotCount; i++) {
        ...
        await waitForOutputReady(shotImageId as string)
  ...
  // 多角度分镜工作流
  const executeMultiAngleStoryboard = async (multiAngle: any, position: any) => {
    ...
    try {
      ...
      const createdAngles: any[] = []
      const ax = position.x + sp + 100

      for (let i = 0; i < angles.length; i++) {
        const key = angles[i]
        const cfg = (MULTI_ANGLE_PROMPTS as any)[key]
  ...
  // 根据工作流类型执行
  const executeWorkflow = async (params: any, position: any) => {
