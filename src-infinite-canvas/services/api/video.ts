...
import { referenceImagesToSingleFile } from "@infinite/lib/image-utils";
...
type RequestOptions = { signal?: AbortSignal; onTaskCreated?: (task: CanvasMindTaskCreated) => void | Promise<void> };
...
export async function requestVideoGeneration(config: AiConfig, prompt: string, references: ReferenceImage[] = [], videoReferences: ReferenceVideo[] = [], audioReferences: ReferenceAudio[] = [], options?: RequestOptions): Promise<VideoGenerationResult> {
    if (shouldUseCanvasMindModelRuntime(config, "video")) {
        ...
        return { url: await requestCanvasMindVideo(config, prompt, imageUrls, mediaReferences, options) };
    }
...
export async function recoverGeneratedVideoFromUrl(config: AiConfig, url: string, options?: RequestOptions): Promise<UploadedFile> {
    const selectedModel = (config.model || config.videoModel).trim();
    const requestConfig = resolveModelRequestConfig(config, selectedModel);
    assertVideoConfig(requestConfig, requestConfig.model);
    return storeGeneratedVideo(await videoResultFromUrl(requestConfig, url, options));
}

async function createOpenAIVideoTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], options?: RequestOptions): Promise<VideoGenerationTask> {
    const body = new FormData();
    body.append("model", modelOptionName(model));
    body.append("prompt", openAIVideoPrompt(prompt, references));
    body.append("seconds", normalizeVideoSeconds(config.videoSeconds));
    if (normalizeVideoSize(config.size)) body.append("size", normalizeVideoSize(config.size)!);
    const referenceFile = await referenceImagesToSingleFile(
        await Promise.all(references.slice(0, 9).map(async (image) => ({ ...image, dataUrl: await imageToDataUrl(image) }))),
    );
    if (referenceFile) body.append("input_reference", referenceFile);
    try {
...
function openAIVideoPrompt(prompt: string, references: ReferenceImage[]) {
    if (references.length <= 1) return prompt;
    return `输入参考图是一张由 ${references.length} 张角色设定图组成的参考板，按从左到右、从上到下依次标为“参考图 1”至“参考图 ${references.length}”。请严格保持每张参考图中角色的脸部、发型、服装和配色设定。\n\n${prompt}`;
}
...
function shouldAuthorizeVideoResult(config: AiConfig, url: string) {
    try {
...
async function videoResultFromUrl(config: AiConfig, url: string, options?: RequestOptions): Promise<VideoGenerationResult> {
    const requiresProviderAuthorization = shouldAuthorizeVideoResult(config, url);
    try {
        const response = await axios.get<Blob>(url, {
            headers: requiresProviderAuthorization ? aiHeaders(config) : undefined,
            responseType: "blob",
            signal: options?.signal,
        });
        ...
        // A provider-owned content endpoint cannot be used as a raw <video> URL because
        // media elements cannot attach the configured Bearer token. Surface the download
        // failure instead of persisting a result that is guaranteed to be unplayable.
        if (requiresProviderAuthorization) throw error;
