            ...
            <div class="header-2wr">
              <div class="container-txy">
                <div
                  ...
                  @click="emit('set-audio-filter', option.value)"
                >
                  {{ option.label }}
                </div>
              </div>
...
import type { AudioFilterType, FilterOption } from '@/views/asset/types'

defineProps<{
  active: boolean
  audioFilterOptions: FilterOption<AudioFilterType>[]
  audioFilter: AudioFilterType
}>()

const emit = defineEmits<{
  'set-audio-filter': [filter: AudioFilterType]
  'enter-batch-mode': []
  'edit-in-capcut': []
}>()
</script>
