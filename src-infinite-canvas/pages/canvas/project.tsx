...
import { requestEdit, requestGeneration, requestImageQuestion } from "@infinite/services/api/image";
import { requestAudioGeneration, storeGeneratedAudio } from "@infinite/services/api/audio";
import { recoverGeneratedVideoFromUrl, requestVideoGeneration, storeGeneratedVideo } from "@infinite/services/api/video";
import { resumeCanvasMindGenerationTask, type CanvasMindGenerationRecord, type CanvasMindTaskCreated } from "@infinite/services/canvasmind-model-runtime";
import { defaultConfig, useConfigStore, useEffectiveConfig } from "@infinite/stores/use-config-store";
import { uploadImage } from "@infinite/services/image-storage";
import { uploadMediaFile } from "@infinite/services/file-storage";
import { nanoid } from "nanoid";
import { getDataUrlByteSize, readImageMeta } from "@infinite/lib/image-utils";
import { canvasThemes, type CanvasBackgroundMode } from "@infinite/lib/canvas-theme";
import { useAssetStore } from "@infinite/stores/use-asset-store";
import { useThemeStore } from "@infinite/stores/use-theme-store";
import { cropDataUrl, splitDataUrl, upscaleDataUrl } from "@infinite/lib/canvas/canvas-image-data";
import { fitNodeSize, nodeSizeFromRatio } from "@infinite/lib/canvas/canvas-node-size";
import { App, Button, Input, Modal } from "antd";
import { NODE_DEFAULT_SIZE, getNodeSpec } from "@infinite/constant/canvas";
import { ActiveConnectionPath, ConnectionPath } from "@infinite/components/canvas/canvas-connections";
import { CanvasConfigComposer } from "@infinite/components/canvas/canvas-config-composer";
import { CanvasConfigNodePanel } from "@infinite/components/canvas/canvas-config-node-panel";
import { CanvasNodeContextMenu } from "@infinite/components/canvas/canvas-context-menu";
import { CanvasNodeAngleDialog, type CanvasImageAngleParams } from "@infinite/components/canvas/canvas-node-angle-dialog";
import { CanvasNodeCropDialog, type CanvasImageCropRect } from "@infinite/components/canvas/canvas-node-crop-dialog";
import { CanvasNodeMaskEditDialog, type CanvasImageMaskEditPayload } from "@infinite/components/canvas/canvas-node-mask-edit-dialog";
import { CanvasNodeSplitDialog, type CanvasImageSplitParams } from "@infinite/components/canvas/canvas-node-split-dialog";
import { CanvasNodeUpscaleDialog, type CanvasImageUpscaleParams } from "@infinite/components/canvas/canvas-node-upscale-dialog";
import { buildNodeGenerationContext, buildNodeGenerationInputs, buildNodeResponseMessages, hydrateNodeGenerationContext, type NodeGenerationInput } from "@infinite/components/canvas/canvas-node-generation";
import { CanvasNodeHoverToolbar, CanvasNodeInfoModal } from "@infinite/components/canvas/canvas-node-hover-toolbar";
import { InfiniteCanvas } from "@infinite/components/canvas/infinite-canvas";
import { Minimap } from "@infinite/components/canvas/canvas-mini-map";
import { CanvasNode } from "@infinite/components/canvas/canvas-node";
import { CanvasNodePromptPanel, type CanvasNodeGenerationMode } from "@infinite/components/canvas/canvas-node-prompt-panel";
import { CanvasToolbar } from "@infinite/components/canvas/canvas-toolbar";
import { AssetPickerModal, type InsertAssetPayload } from "@infinite/components/canvas/asset-picker-modal";
import { CanvasZoomControls } from "@infinite/components/canvas/canvas-zoom-controls";
import { useAgentStore } from "@infinite/stores/use-agent-store";
import { flushCanvasStorePersistence, useCanvasStore } from "@infinite/stores/canvas/use-canvas-store";
import { useAgentBridge } from "@infinite/pages/canvas/hooks/use-agent-bridge";
import { usePluginHost } from "@infinite/pages/canvas/hooks/use-plugin-host";
import { buildNodeMentionReferences, type CanvasResourceReference } from "@infinite/lib/canvas/canvas-resource-references";
import { applyNodeConfigPatch, audioMetadata, buildAudioGenerationMetadata, buildImageGenerationMetadata, createCanvasNode, imageMetadata, videoMetadata } from "@infinite/lib/canvas/canvas-node-factory";
import { findContainingGroupId, findGroupDropTarget, getConnectionTargetAnchor, normalizeConnection, snapNodesIntoGroup } from "@infinite/lib/canvas/canvas-node-geometry";
import {
    audioExtension,
    buildAngleLabel,
    buildAnglePrompt,
...
} from "@infinite/lib/canvas/canvas-generation-helpers";
import { getNodeDefinition, isBuiltinNodeType as isBuiltinType, useNodeRegistryVersion } from "@infinite/lib/canvas/node-registry";
import { registerBuiltinNodes } from "@infinite/components/canvas/nodes/builtin-nodes";
import { CanvasRefreshShell } from "@infinite/components/canvas/canvas-refresh-shell";
import { ConnectionCreateMenu, NodeCreateMenu, type PendingConnectionCreate } from "@infinite/components/canvas/canvas-create-menus";
import {
    CanvasNodeType,
...
} from "@infinite/types/canvas";
import type { ReferenceImage } from "@infinite/types/image";
import type { ReferenceAudio } from "@infinite/types/media";
...
function generationRecordError(record: CanvasMindGenerationRecord) {
    if (record.stopped) return "生成任务已停止";
    return record.error || "生成任务完成但没有返回可用结果";
}

async function recoverPersistedCanvasNode(node: CanvasNodeData, signal: AbortSignal): Promise<CanvasNodeData> {
    const pendingImages = node.metadata?.images?.filter((image) => image.status === NODE_STATUS_LOADING && image.generationTaskId) || [];
    if (pendingImages.length) {
        const recovered = await Promise.all(
            pendingImages.map(async (pendingImage) => {
                try {
                    const record = await resumeCanvasMindGenerationTask(pendingImage.generationTaskId!, { signal });
                    const url = record.images?.find(Boolean);
                    if (!url) throw new Error(generationRecordError(record));
                    ...
                    return {
                        ...
                        errorDetails: error instanceof Error ? error.message : "恢复生成任务失败",
                        generationTaskId: undefined,
                    } satisfies CanvasNodeImage;
                }
            }),
        );
        const recoveredById = new Map(recovered.map((image) => [image.id, image]));
        const images =
            node.metadata?.images?.map((image) =>
                recoveredById.get(image.id) ||
                (image.status === NODE_STATUS_LOADING
                    ? { ...image, status: NODE_STATUS_ERROR, errorDetails: "页面刷新前未保存生成任务标识，请重新生成。", generationTaskId: undefined }
                    : image),
            ) || recovered;
        const successfulImages = images.filter((image) => image.status === NODE_STATUS_SUCCESS && image.content);
        const primaryImage = images.find((image) => image.id === node.metadata?.primaryImageId && image.status === NODE_STATUS_SUCCESS) || successfulImages[0];
        const firstError = recovered.find((image) => image.status === NODE_STATUS_ERROR)?.errorDetails;
        if (!primaryImage) {
            return { ...node, metadata: { ...node.metadata, images, status: NODE_STATUS_ERROR, errorDetails: firstError || "全部图片生成失败" } };
        }
        const size = fitNodeSize(primaryImage.naturalWidth, primaryImage.naturalHeight, Math.max(node.width, NODE_DEFAULT_SIZE[CanvasNodeType.Image].width), Math.max(node.height, NODE_DEFAULT_SIZE[CanvasNodeType.Image].height));
        return {
            ...node,
            width: size.width,
            height: size.height,
    ...
    try {
        const record = await resumeCanvasMindGenerationTask(taskId, { signal });
        const imageUrl = record.images?.find(Boolean);
        if (imageUrl) {
            const uploaded = await uploadImage(imageUrl);
            const size = fitNodeSize(uploaded.width, uploaded.height, Math.max(node.width, NODE_DEFAULT_SIZE[CanvasNodeType.Image].width), Math.max(node.height, NODE_DEFAULT_SIZE[CanvasNodeType.Image].height));
            return {
                ...node,
                width: size.width,
                height: size.height,
                position: { x: node.position.x + node.width / 2 - size.width / 2, y: node.position.y + node.height / 2 - size.height / 2 },
                metadata: { ...node.metadata, ...imageMetadata(uploaded), generationTaskId: undefined, errorDetails: undefined },
            };
        }
        const videoUrl = record.outputs?.find((output) => output.outputType === "video" && output.url)?.url;
        if (videoUrl) {
            const video = await storeGeneratedVideo({ url: videoUrl });
            ...
            return {
                ...
                height: size.height,
                position: { x: node.position.x + node.width / 2 - size.width / 2, y: node.position.y + node.height / 2 - size.height / 2 },
                metadata: { ...node.metadata, ...videoMetadata(video), generationTaskId: undefined, errorDetails: undefined },
            };
        }
        const content = String(record.content || record.outputs?.find((output) => output.textContent)?.textContent || "");
        if (content) {
            return { ...node, type: node.type === CanvasNodeType.Config ? CanvasNodeType.Text : node.type, metadata: { ...node.metadata, content, status: NODE_STATUS_SUCCESS, generationTaskId: undefined, errorDetails: undefined } };
        }
        throw new Error(generationRecordError(record));
    } catch (error) {
        if (signal.aborted) throw error;
        return { ...node, metadata: { ...node.metadata, status: NODE_STATUS_ERROR, generationTaskId: undefined, errorDetails: error instanceof Error ? error.message : "恢复生成任务失败" } };
    }
}
...
function InfiniteCanvasPage() {
    ...
    const config = useConfigStore((state) => state.runtimeConfig || state.config);
    ...
    const [recoverVideoUrl, setRecoverVideoUrl] = useState("");
    const [recoveringVideo, setRecoveringVideo] = useState(false);
    ...
    useEffect(() => {
        if (!currentProject?.title || window.parent === window) return;
        window.parent.postMessage({ type: "canvasmind:project-meta", title: currentProject.title }, window.location.origin);
    ...
    const rememberGenerationTask = useCallback(async (targetNodeId: string, task: CanvasMindTaskCreated, imageId?: string) => {
        const applyTaskId = (sourceNodes: CanvasNodeData[]) =>
                ...
                if (imageId) {
                    return {
                        ...
                        metadata: {
                            ...node.metadata,
                            generationTaskId: undefined,
                            images: node.metadata?.images?.map((image) => (image.id === imageId ? { ...image, generationTaskId: task.id } : image)),
                        },
                    };
                }
                return { ...node, metadata: { ...node.metadata, generationTaskId: task.id } };
            });
        setNodes(applyTaskId);
        const storedProject = useCanvasStore.getState().projects.find((project) => project.id === projectId);
        const persistedNodes = storedProject?.nodes.some((node) => node.id === targetNodeId) ? storedProject.nodes : nodesRef.current;
        if (!persistedNodes.some((node) => node.id === targetNodeId)) return;
        updateProject(projectId, { nodes: applyTaskId(persistedNodes) });
        await flushCanvasStorePersistence();
    }, [projectId, updateProject]);

    const generationRequestOptions = useCallback(
        (targetNodeId: string, controller: AbortController, imageId?: string) => ({
            signal: controller.signal,
            onTaskCreated: (task: CanvasMindTaskCreated) => rememberGenerationTask(targetNodeId, task, imageId),
        }),
        [rememberGenerationTask],
    );

    const recoverPersistedGenerations = useCallback(async (initialNodes: CanvasNodeData[], initialConnections: CanvasConnection[], signal: AbortSignal) => {
        const pendingNodes = initialNodes.filter(hasRecoverableGenerationTask);
        await Promise.all(
            pendingNodes.map(async (pendingNode) => {
                const recoveredNode = await recoverPersistedCanvasNode(pendingNode, signal);
                if (signal.aborted) return;
                setNodes((prev) => prev.map((node) => (node.id === recoveredNode.id ? recoveredNode : node)));
            }),
        );
        if (signal.aborted) return;
        const recoveredChildIds = new Set(pendingNodes.map((node) => node.id));
        const originIds = new Set(initialConnections.filter((connection) => recoveredChildIds.has(connection.toNodeId)).map((connection) => connection.fromNodeId));
        setNodes((prev) =>
            prev.map((node) => {
                if (!originIds.has(node.id) || node.metadata?.status !== NODE_STATUS_LOADING) return node;
                const childIds = initialConnections.filter((connection) => connection.fromNodeId === node.id && recoveredChildIds.has(connection.toNodeId)).map((connection) => connection.toNodeId);
                const children = prev.filter((child) => childIds.includes(child.id));
                const hasSuccess = children.some((child) => child.metadata?.status === NODE_STATUS_SUCCESS);
                const firstError = children.find((child) => child.metadata?.errorDetails)?.metadata?.errorDetails;
                return { ...node, metadata: { ...node.metadata, status: hasSuccess ? NODE_STATUS_SUCCESS : NODE_STATUS_ERROR, errorDetails: hasSuccess ? undefined : firstError || "生成任务恢复失败" } };
            }),
        );
    }, []);
    ...
    useEffect(() => {
        ...
        if (!project && searchParams.get("source") === "canvasmind") {
            const projectTitle = searchParams.get("title")?.trim() || t("canvas.project.untitled");
            createProject(projectTitle, projectId);
        ...
        const restore = async () => {
            ...
            void recoverPersistedGenerations(restoredNodes, project.connections, recoveryController.signal);
        };
        void restore();
        return () => recoveryController.abort();
    }, [createProject, hydrated, navigate, openProject, projectId, recoverPersistedGenerations, searchParams, t]);
    ...
    const maskEditImageNode = useCallback(
        async (node: CanvasNodeData, payload: CanvasImageMaskEditPayload) => {
            ...
            try {
                const image = await requestEdit(generationConfig, prompt, [source], { id: `${node.id}-mask`, name: "mask.png", type: "image/png", dataUrl: payload.maskDataUrl }, generationRequestOptions(childId, controller)).then((items) => items[0]);
        ...
        [effectiveConfig, finishGenerationRequest, generationRequestOptions, isAiConfigReady, message, openConfigDialog, startGenerationRequest, t],
    );
    ...
    const generateAngleNode = useCallback(
        async (node: CanvasNodeData, params: CanvasImageAngleParams) => {
            ...
            try {
                const image = await requestEdit(
                    ...
                    [{ id: node.id, name: `${node.title || node.id}.png`, type: node.metadata.mimeType || "image/png", dataUrl: node.metadata.content, storageKey: node.metadata.storageKey }],
                    undefined,
                    generationRequestOptions(childId, controller),
                ).then((items) => items[0]);
                const uploaded = await uploadImage(image.dataUrl);
        ...
        [effectiveConfig, finishGenerationRequest, generationRequestOptions, openConfigDialog, startGenerationRequest, t],
    );
            ...
            if (sourceNode && builtinPanel?.writeBackToSelf && builtinPanel.mode === "image") {
                ...
                try {
                    ...
                    const image = refs.length
                        ? await requestEdit({ ...generationConfig, count: "1" }, fullPrompt, refs, undefined, generationRequestOptions(nodeId, controller)).then((items) => items[0])
                        : await requestGeneration({ ...generationConfig, count: "1" }, fullPrompt, generationRequestOptions(nodeId, controller)).then((items) => items[0]);
                    const uploaded = await uploadImage(image.dataUrl);
                    setNodes((prev) =>
                        ...
                        imageIds.map(async (imageId) => {
                            try {
                                const image = referenceImages.length
                                    ? await requestEdit({ ...generationConfig, count: "1" }, effectivePrompt, referenceImages, undefined, generationRequestOptions(rootId, controller, imageId)).then((items) => items[0])
                                    : await requestGeneration({ ...generationConfig, count: "1" }, effectivePrompt, generationRequestOptions(rootId, controller, imageId)).then((items) => items[0]);
                                const uploaded = await uploadImage(image.dataUrl);
                ...
                if (mode === "video") {
                    ...
                    try {
                        const video = await storeGeneratedVideo(
                            await requestVideoGeneration(generationConfig, effectivePrompt, generationContext.referenceImages, generationContext.referenceVideos, generationContext.referenceAudios, generationRequestOptions(videoId, controller)),
                        );
                ...
                const answers = await Promise.all(
                    textTargetIds.map((targetNodeId) => {
                        ...
                        return requestImageQuestion(
                            ...
                            generationRequestOptions(targetNodeId, controller),
                        )
            ...
            try {
                if (node.type === CanvasNodeType.Text) {
                    ...
                    const answer = await requestImageQuestion(
                        ...
                        generationRequestOptions(node.id, controller),
                    );
                    setNodes((prev) => prev.map((item) => (item.id === node.id ? { ...item, type: CanvasNodeType.Text, metadata: { ...item.metadata, content: answer || streamed, prompt, status: NODE_STATUS_SUCCESS } } : item)));
                    return;
                }
                if (node.type === CanvasNodeType.Video) {
                    const video = await storeGeneratedVideo(await requestVideoGeneration(generationConfig, prompt, retryImages, context?.referenceVideos || [], context?.referenceAudios || [], generationRequestOptions(node.id, controller)));
                ...
                const image = useReferenceImages
                    ? await requestEdit(generationConfig, prompt, retryImages, undefined, generationRequestOptions(node.id, controller, imageId)).then((items) => items[0])
                    : await requestGeneration(generationConfig, prompt, generationRequestOptions(node.id, controller, imageId)).then((items) => items[0]);
    ...
    const recoverVideoPreview = useCallback(async () => {
        const node = nodesRef.current.find((item) => item.id === recoverVideoNodeId && item.type === CanvasNodeType.Video);
        const url = recoverVideoUrl.trim();
        if (!node || !/^https?:\/\//i.test(url)) {
            message.warning("请输入有效的视频 URL");
            return;
        }
        const sourceNode = findRetrySourceNode(node.id, nodesRef.current, connectionsRef.current) || node;
        const generationConfig = buildGenerationConfig(effectiveConfig, sourceNode, "video");
        if (!isAiConfigReady(generationConfig, generationConfig.model)) {
            openConfigDialog(true);
            return;
        }

        setRecoveringVideo(true);
        try {
            const video = await recoverGeneratedVideoFromUrl(generationConfig, url);
            const videoSize = fitNodeSize(video.width || node.width, video.height || node.height, VIDEO_NODE_MAX_WIDTH, VIDEO_NODE_MAX_HEIGHT);
            setNodes((prev) => prev.map((item) => item.id === node.id ? {
                ...
                height: videoSize.height,
                position: { x: item.position.x + item.width / 2 - videoSize.width / 2, y: item.position.y + item.height / 2 - videoSize.height / 2 },
                metadata: { ...item.metadata, ...videoMetadata(video), sourceUrl: url },
            } : item));
            setRecoverVideoNodeId(null);
            setRecoverVideoUrl("");
            message.success("视频预览已恢复");
        } catch (error) {
            message.error(error instanceof Error ? error.message : "视频预览恢复失败");
        } finally {
            setRecoveringVideo(false);
        }
    }, [effectiveConfig, isAiConfigReady, message, openConfigDialog, recoverVideoNodeId, recoverVideoUrl]);
    ...
    return (
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            <section className="relative min-w-0 flex-1 overflow-hidden">
                ...
                <Modal
                    title="使用已有 URL 恢复视频预览"
                    open={Boolean(recoverVideoNodeId)}
                    centered
                    onCancel={() => {
                    ...
                    footer={
                        <>
                            <Button disabled={recoveringVideo} onClick={() => {
                                setRecoverVideoNodeId(null);
                                setRecoverVideoUrl("");
                            }}>取消</Button>
                            <Button type="primary" loading={recoveringVideo} onClick={() => void recoverVideoPreview()}>恢复预览</Button>
                        </>
                    }
                >
                    <p className="mb-3 text-sm opacity-60">不会重新生成视频；将使用当前模型渠道鉴权下载并写回这个节点。</p>
                    <Input.TextArea
                        value={recoverVideoUrl}
                        autoSize={{ minRows: 3, maxRows: 5 }}
                        placeholder="https://.../videos/task-id/content"
                        onChange={(event) => setRecoverVideoUrl(event.target.value)}
                    />
                </Modal>
