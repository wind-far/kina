...
import { canvasThemes, type CanvasBackgroundMode, type CanvasColorTheme, type CanvasTheme } from "@infinite/lib/canvas-theme";
import { selectableModelsByCapability, useEffectiveConfig } from "@infinite/stores/use-config-store";
import { getNodePluginId, listNodeDefinitions, useNodeRegistryVersion } from "@infinite/lib/canvas/node-registry";
import { useThemeStore } from "@infinite/stores/use-theme-store";
import { AnimatedThemeToggler } from "@infinite/components/ui/animated-theme-toggler";
...
export function CanvasToolbar({
    ...
    const hasImageModel = selectableModelsByCapability(modelConfig, "image").length > 0;
    const hasVideoModel = selectableModelsByCapability(modelConfig, "video").length > 0;
    const hasAudioModel = selectableModelsByCapability(modelConfig, "audio").length > 0;
...
function ToolbarButton({
    ...
    return (
        <Button
            ...
            disabled={disabled}
            style={active ? activeStyle : hovered === id && !disabled ? hoverStyle : { color: danger ? "var(--functional-error, #f35)" : theme.toolbar.item, opacity: disabled ? 0.35 : 1 }}
            icon={children}
            onMouseEnter={(event) => {
                onHover(id);
