...
import { canvasThemes } from "@infinite/lib/canvas-theme";
import { formatBytes } from "@infinite/lib/image-utils";
import { getNodeDefinition } from "@infinite/lib/canvas/node-registry";
import { buildNodeContext } from "@infinite/lib/canvas/plugin-node-context";
import { useThemeStore } from "@infinite/stores/use-theme-store";
import { CanvasResourceMentionTextarea } from "./canvas-resource-mention-textarea";
import { CanvasNodeType, type CanvasNodeData, type CanvasNodeImage, type Position } from "@infinite/types/canvas";
import type { CanvasNodeContext, CanvasPluginHost } from "@infinite/types/canvas-plugin";
import type { CanvasResourceReference } from "@infinite/lib/canvas/canvas-resource-references";
import { useTranslation } from "react-i18next";

type ResizeCorner = "top-left" | "top-right" | "bottom-left" | "bottom-right";
const selectionBlue = "var(--canvas-selection-border, #00a1c2)";
    ...
    return (
        ...
        >
            ...
            <div
                ...
                style={{
                    background: isGroup ? `color-mix(in srgb, ${theme.toolbar.panel} 40%, transparent)` : hasImageContent || hasVideoContent || transparentBg ? "transparent" : theme.node.fill,
                    ...
                    boxShadow: isGroupDropTarget ? `0 0 0 2px color-mix(in srgb, ${selectionBlue} 40%, transparent), inset 0 0 0 999px color-mix(in srgb, ${selectionBlue} 6%, transparent)` : isActive ? `0 0 0 1px color-mix(in srgb, ${selectionBlue} 34%, transparent)` : isRelated ? `0 0 0 1px color-mix(in srgb, ${theme.node.muted} 34%, transparent), 0 18px 48px rgba(0,0,0,.14)` : undefined,
                }}
            ...
            >
                ...
                {!isGroup && !hasImageContent && !hasVideoContent && !hasAudioContent ? <div className="pointer-events-none absolute inset-x-0 bottom-0 h-12" style={{ background: `linear-gradient(to top, color-mix(in srgb, ${theme.canvas.background} 40%, transparent), transparent)` }} /> : null}
...
function GroupNodeContent({ node, theme, groupChildCount }: NodeContentRendererProps) {
    ...
    return (
        <div className="pointer-events-none flex h-full w-full flex-col p-4">
            ...
            <div className="mt-3 flex-1 rounded-2xl border border-dashed" style={{ borderColor: theme.node.stroke, background: `color-mix(in srgb, ${theme.node.fill} 34%, transparent)` }} />
        </div>
    );
}
    ...
    return (
        <div className="flex h-full w-full flex-col overflow-hidden pt-8">
            <button
                ...
                style={{ background: `color-mix(in srgb, ${theme.toolbar.panel} 87%, transparent)`, borderColor: theme.node.stroke, color: theme.node.text }}
                onClick={(event) => {
                    event.stopPropagation();
...
function VideoNodeContent({ node, theme }: NodeContentRendererProps) {
    ...
    const initializeVideoAudio = (video: HTMLVideoElement) => {
    ...
    return (
        <video
            ...
            data-canvas-no-zoom
            onLoadedMetadata={(event) => initializeVideoAudio(event.currentTarget)}
        />
    );
}
