import { defaultConfig, resolveModelForCapability, type AiConfig } from "@infinite/stores/use-config-store";
import i18n from "@infinite/i18n";
import { resolveImageUrl, uploadImage } from "@infinite/services/image-storage";
import { resolveMediaUrl } from "@infinite/services/file-storage";
import { imageMetadata, referenceUrl } from "@infinite/lib/canvas/canvas-node-factory";
import type { NodeGenerationInput } from "@infinite/components/canvas/canvas-node-generation";
import type { CanvasNodeGenerationMode } from "@infinite/components/canvas/canvas-node-prompt-panel";
import type { CanvasImageAngleParams } from "@infinite/components/canvas/canvas-node-angle-dialog";
import type { ReferenceImage } from "@infinite/types/image";
import { CanvasNodeType, type CanvasAssistantSession, type CanvasConnection, type CanvasNodeData, type CanvasNodeMetadata } from "@infinite/types/canvas";
...
export function hasRecoverableGenerationTask(node: CanvasNodeData) {
    if (node.metadata?.generationTaskId) return true;
    return Boolean(node.metadata?.images?.some((image) => image.status === "loading" && image.generationTaskId));
}

export function resetInterruptedGeneration(nodes: CanvasNodeData[], connections: CanvasConnection[] = []) {
    const recoverableNodeIds = new Set(nodes.filter(hasRecoverableGenerationTask).map((node) => node.id));
    const recoverableOriginIds = new Set(connections.filter((connection) => recoverableNodeIds.has(connection.toNodeId)).map((connection) => connection.fromNodeId));
    return nodes.map((node) =>
        node.metadata?.status === "loading" && !recoverableNodeIds.has(node.id) && !recoverableOriginIds.has(node.id)
            ? {
                  ...node,
                  metadata: {
                      ...node.metadata,
