...
import { audioMimeType, normalizeAudioFormatValue, normalizeAudioSpeedValue, normalizeAudioVoiceValue } from "@infinite/lib/audio-generation";
import { uploadMediaFile, type UploadedFile } from "@infinite/services/file-storage";
import { buildApiUrl, resolveModelRequestConfig, resolveModelScript, type AiConfig } from "@infinite/stores/use-config-store";
import { runModelPlugin } from "./model-plugin";
import { shouldUseCanvasMindModelRuntime } from "@infinite/services/canvasmind-model-runtime";
...
export async function requestAudioGeneration(config: AiConfig, prompt: string, options?: RequestOptions): Promise<Blob> {
    if (shouldUseCanvasMindModelRuntime(config, "audio")) throw new Error("CanvasMind 本地模型目录当前不提供音频生成模型");
