import {
	IndexedDBAdapter,
	deleteDatabase,
} from "@cutia/services/storage/indexeddb-adapter";
...
export async function runStorageMigrations({
	...
	console.log("[CutiaPOC] runStorageMigrations enter");
	// One-time cleanup: delete the old global version database
	if (!hasCleanedUpMetaDb) {
		try {
			console.log("[CutiaPOC] deleteDatabase video-editor-meta ...");
			await deleteDatabase({ dbName: "video-editor-meta" });
			console.log("[CutiaPOC] deleteDatabase done");
		} catch (e) {
			console.log("[CutiaPOC] deleteDatabase failed (ignored):", e);
		}
		hasCleanedUpMetaDb = true;
	}

	console.log("[CutiaPOC] open projectsAdapter for migration scan");
	...
	console.log("[CutiaPOC] projects.getAll done, count=", projects.length);
