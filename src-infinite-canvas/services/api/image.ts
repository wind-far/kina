...
import { buildApiUrl, resolveModelRequestConfig, resolveModelScript, type AiConfig, type ModelChannel } from "@infinite/stores/use-config-store";
import { normalizePluginImages, runModelPlugin } from "./model-plugin";
import { nanoid } from "nanoid";
import { dataUrlToFile } from "@infinite/lib/image-utils";
import { buildImageReferencePromptText } from "@infinite/lib/image-reference-prompt";
import { imageToDataUrl } from "@infinite/services/image-storage";
import type { ReferenceImage } from "@infinite/types/image";
import { requestCanvasMindImages, requestCanvasMindText, shouldUseCanvasMindModelRuntime, type CanvasMindTaskCreated } from "@infinite/services/canvasmind-model-runtime";
...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    if (shouldUseCanvasMindModelRuntime(config, "image")) {
        const images = await requestCanvasMindImages(config, prompt, [], options);
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    if (shouldUseCanvasMindModelRuntime(config, "image")) {
        if (mask) throw new Error("本地模型服务暂不支持画布蒙版编辑");
        const referenceImages = await Promise.all(references.map((image) => imageToDataUrl(image)));
        const images = await requestCanvasMindImages(config, prompt, referenceImages, options);
        return images.map((dataUrl) => ({ id: nanoid(), dataUrl }));
    }
    const requestConfig = resolveModelRequestConfig(config, config.model || config.imageModel);
    const n = Math.max(1, Math.min(15, Math.floor(Math.abs(Number(config.count)) || 1)));
...
export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    if (shouldUseCanvasMindModelRuntime(config, "text")) return requestCanvasMindText(config, messages, onDelta, options);
